...
function rho2sdf(
    ...
    if options.export_raw_sdf
        ...
        # Export raw SDF to JLD2 in the same format as smoothed results
        println("Saving raw SDF results to JLD2 files...")

        # Process sdf_dists to handle extreme values (same as in RBFs_smoothing)
        sdf_dists_processed = SdfSmoothing.process_vector(sdf_dists)

        # Convert to 3D array (same format as fine_sdf)
        raw_sdf_dims = sdf_grid.N .+ 1
        raw_sdf = reshape(sdf_dists_processed, Tuple(raw_sdf_dims))

        # Create grid coordinates array (same format as fine_grid)
        raw_grid = SdfSmoothing.create_grid(sdf_grid.N, sdf_grid)

        @save "Z_$(taskName)_$(element_name)_RawSDF_B-$(B).jld2" raw_sdf
        @save "Z_$(taskName)_$(element_name)_RawGrid_B-$(B).jld2" raw_grid
    end
